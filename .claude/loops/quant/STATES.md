...
| `DEGRADED` | El objetivo se cumplió con una limitación no crítica, acotada y registrada. | Se cumplen (a) y (c), pero una validación no aplica por muestra insuficiente, una fuente no crítica no estaba fresca o existe una omisión admitida explícitamente por el loop (p. ej., liga excluida por cuota en el loop 01). Exige nombrar la limitación y el `n` disponible. Nunca se usa para ocultar un comando fallido, una validación crítica fallida ni un artefacto obligatorio ausente. |
