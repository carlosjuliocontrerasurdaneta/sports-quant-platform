...
- Status: PARCIAL (revisado el 2026-09-04). (a) **RESUELTO** el 2026-09-02 (commit `f173d5a`, AUD-HIGH-001): `train_market_calibrators` colapsa a observaciones independientes y evalua `if n_events < min_n`; `calibration/data.py` transporta `selection` y `line` para que la unidad (event_id, selection, line) agrupe lo mismo que el dedup. (b) **ABIERTO**: `evaluation/model_vs_market.py:114-141` sigue puntuando sobre el stream inflado -- el bootstrap agrupa por `event_id`, pero el ESTIMADOR PUNTUAL pondera cada pick por su numero de dias de horizonte, que varia sistematicamente. `edge_information.one_row_per_pick` existe exactamente para eso y no se usa. Corregirlo altera cifras publicadas. (c) **MITIGADO** el 2026-09-01: `monitoring/health.py:267` anade el inventario por liga SERVIDA junto al universo fijo `ML_LEAGUES`; lo que sigue sin cerrarse es aseverar `calibration`/`totals_model`/`model_age_days` en vez de solo registrarlos. Evidencia en `audit/latest/FINDINGS.md` (N4-A-6, N4-A-8, N4-A-9).
...
- Proposed fix: NO se corrige unilateralmente. El fallback es la decision del 2026-08-28 (leccion de los 53 dias: un tablero en blanco hizo creer al operador que el sistema no generaba nada) y esta fijado por `test_cae_al_dia_mas_reciente_si_hoy_no_hay`. Cambiarlo contradice una decision registrada y exige autorizacion explicita del operador. Una salida posible sin tocar la decision: marcar visiblemente esas filas como "en juego" en vez de suprimirlas.
- Status: Abierto a proposito (2026-09-02; alcance re-medido el 2026-09-04). Con horizonte de 7 dias hacen falta CERO filas apostables en todo el stream para alcanzar el fallback, asi que hoy no se dispara. Registrado aqui el 2026-09-04: vivia solo en un docstring y en un test, invisible para cualquiera que leyera este registro para saber que sigue abierto (AUD-MED-002).
