...
**Pendiente (en `Obsidian/Tareas.md`):** confirmar el 17/09 tras las 12:00 que `DIARIO COMPLETO: OK` reaparece; diagnosticar `SQP_Validate_OOS_Cdev`; tratar la rama por el flujo model-change (pre-registro, medir el efecto de `ordered_games(utc=True)` sobre fechas locales y doubleheaders, champion-challenger) — **no mergear a `main` sin aprobación del operador**.
