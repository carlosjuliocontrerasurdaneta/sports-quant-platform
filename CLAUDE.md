...
- GOVERNING PRINCIPLE for model choice (operator order, restated 2026-09-01 and 2026-09-26 as a fundamental principle, carved in stone): always use the superior model for tasks demanding the highest level of reasoning, and delegate the rest according to their complexity and to where each model performs best. It governs: when a specific rule conflicts with it, the principle wins and the rule gets fixed. Operational split: `claude-opus-5-5` is the default and the starting point (operator order 2026-09-22, superseding `claude-opus-5`, which Anthropic's live docs now list as legacy); Sonnet handles normal engineering work and Haiku bounded lookup/summarization; `claude-fable-5-1` is the ceiling (note the hyphens: `claude-fable-5.1` is not a valid ID, and `claude-fable-5` is a different, now-legacy model), reserved for maximum-reasoning tasks and the destination of the escalation trigger — starting point and ceiling are deliberately separate. Escalation is decided by observable task CLASS, never by self-assessed difficulty: irreversible work, risk/model/strategy/threshold/gate parameters, publishable figures, contradicting a recorded decision, or changing a persisted artifact's contract. When in doubt between two rungs, go up, and record it in `current-task.md`. Default split: Haiku for bounded lookup, Sonnet for implementation, Opus for orchestration and commits, Fable for pre-registration design, verdicts and any escalation-class piece — and Fable reviews BEFORE the commit, never after. No model rung substitutes for a measurement: if the question is empirically resolvable with stored data, measure first. Full policy and its locks: `.claude/automation/MODEL_ROUTING.md`. Editing `CLAUDE.md` does not change the active model of a running session.
- Delegation follows the governing principle above (2026-09-26: it prevails over this rule): delegate by model tier whenever a piece of the task fits a different rung — bounded lookup to Haiku, implementation to Sonnet, escalation-class pieces and reviews to Fable. Do not spawn subagents for work you can finish directly at the right rung (a single read, a one-line answer), and never delegate without passing `model`.
