...
- Model choice guidance is advisory only: prefer Sonnet for normal work, Haiku for bounded lookup/summarization, and `claude-opus-5` for genuinely complex architecture, critical incidents, exhaustive quantitative audits and maximum-reasoning tasks (operator decision 2026-08-30, commit `f9a134a`; supersedes Fable for that role). Do not assume `CLAUDE.md` itself changes the active model.
