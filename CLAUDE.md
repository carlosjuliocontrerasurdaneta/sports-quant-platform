...
- Model choice guidance is advisory only: `claude-opus-5` is the default and the starting point; prefer Sonnet for normal engineering work and Haiku for bounded lookup/summarization; reserve `claude-fable-5` for maximum-reasoning tasks, which is where the escalation trigger points (operator decision 2026-08-30). Do not assume `CLAUDE.md` itself changes the active model.
