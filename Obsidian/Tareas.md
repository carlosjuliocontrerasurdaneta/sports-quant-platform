...
- [ ] 2026-08-05 — **Decidir el estadístico del gate intradía** (mediana vs. alternativa que trate empates). Con 41% de ceros exactos la mediana solo supera 0 si >50% de las filas son estrictamente positivas: el gate puede no dispararse nunca aunque exista señal. Análisis completo en [[Bitácora/2026-08-04]]; recomendación: NO pasarse a la media (es ruido: P(media>0)=0.65, y sin tenis es negativa), sino pre-registrar un test de empates ANTES de acumular más muestra. Decisión del operador.
...
- [ ] #4 fase ofensiva: el gate ya es repetible (`scripts/intraday_gate_analysis.py`, criterios exactos en `sqp/audit/intraday_gate.py`). Primera lectura 2026-08-02: **INSUFICIENTE** (n=22 < 30) pero direccionalmente a favor — CLV intradía media +0.29% / beat 41% vs picks 11:00 −2.08% / 31%; el positivo lo carga el tenis (ATP +3.4%, n=4), MLB plano. El observatorio quedó ampliado a spreads/totals (v2, misma sesión) → n≥30 en días; re-ejecutar el script entonces. PASS exige mediana intradía > 0 Y > mediana 11:00. Ver [[Bitácora/2026-08-02]].
