...
| ~~S-1~~ | ~~**Alerta de fallo del run diario.**~~ **IMPLEMENTADO 2026-07-30:** centinela `logs/last_run_status.json` escrito por los 3 BAT en su rama de error y borrado por `DIARIO_COMPLETO.bat` al terminar OK; `health_check.py` lo eleva a ERROR (exit 1) y el dashboard muestra banner rojo. 19 pruebas. | — | — | Hecho |
| ~~—~~ | ~~**Investigar por qué falló el run del 2026-07-29.**~~ **RESUELTO 2026-07-30:** `SSLError` intermitente contra The Odds API y ESPN durante 2 h 22 min; 3 ligas agotaron reintentos. Origen local, no de proveedor. | — | — | Hecho |

| — | **¿Debe un fallo de red transitorio en N de 16 ligas tumbar el exit code?** Hoy `failures=3` equivale a `failures=16`. El operador decidió el 2026-07-30 **mantener la semántica actual**: la alerta resuelve la invisibilidad, que era el problema real. Reabrir solo si los falsos positivos molestan. | S-1 | S | Ruido en la alerta si la red es inestable. | — | Decidido: sin cambio |
