...
## 7. Remediación 3 (2026-09-24/25): identidad de calendario MLB (AUD-003 en MLB, KI-059)

- **Motivo:** la verificación 2 (`VERIFICATION.md` §8) dejó AUD-003 abierto en MLB, que es la liga con más candidatos liquidados.
- **Versión previa de estos entregables:** `history/remediacion3-20260924T234113Z/`.

**Medición previa** (solo lectura; calendario público de MLB en memoria):
- 705 de 710 eventos MLB servidos quedan a 5 minutos o menos de su partido en el calendario, y el resto a 61 minutos, también correctos.
- El segundo partido del par nunca queda a menos de 181 minutos.
- Por eso no hace falta ningún umbral: basta con el partido más cercano.

**Cambio:**

- `MLBStatsProvider.fetch_schedule`: calendario completo, aplazados incluidos, con `gamePk`, `gameDate` UTC, estado, estado abstracto, `startTimeTBD`, `doubleHeader` y `gameNumber`.
- `storage/schedule_store.py`: fichero nuevo `data/historical/schedule_mlb.csv`. Una fila por **aparición** `(game_id, date)`, escrita bajo lock. Un aplazado recuperado conserva su `gamePk` en dos fechas.
- `scripts/backfill_results.py`: para MLB descarga calendario y resultados y los escribe juntos. Si falla el calendario, no se escribe ninguno de los dos.
- `runner.mlb_schedule_scores_map` elige la aparición del mismo par más cercana en hora, y solo gradúa si cumple todo esto:
  - es la única más cercana;
  - cae en la misma fecha ET que el pick;
  - ningún partido del par ese día tiene la hora por confirmar (`startTimeTBD`);
  - la aparición está terminada (`abstract_state == "Final"`) y no está aplazada, cancelada ni suspendida;
  - hay emparejamiento uno a uno frente a los eventos servidos;
  - existe marcador con el mismo `gamePk`. En un suspendido y reanudado vale el de la fecha más tardía.
- Se usa en candidatos y en el stream servido de MLB. Sin calendario devuelve `{}`, y MLB expira como antes.
- `exact_history_scores_map` (identificación por fecha) no acepta MLB.

**Revisión Fable:**

| Ronda | Veredicto | Detalle |
|---|---|---|
| 1 | NO APTO | FABLE-K-001 (ALTA, reproducido con datos reales BOS–BAL y CHC–MIL de 2025): el upsert por `game_id` borraba la aparición aplazada y el aplazado se graduaba con el marcador del otro juego del doubleheader. K-002: suspendido y reanudado. K-003: uno a uno y hora por confirmar |
| 2 | **APTO** | Datos reales pasando por el store: BOS–BAL y CHC–MIL `{}`; ATL–SF suspendido `(2,7)` oficial; KC–COL con hora por confirmar `{}` |

**Validación con datos reales a través del store:** 667 coinciden y 0 no coinciden; 25 sin graduar, que expiran.

**REG-001 (verificación 2):** test nuevo de integración del stream servido ESPN con el resultado a −1 día. Falla con el ±1 de `6676e9a`.

**Estado:**
- **AUD-003: IMPLEMENTADO** (ESPN y MLB), pendiente de verificación independiente.
- **KI-059: implementado.** Se activa cuando el backfill semanal cree `schedule_mlb.csv`.

**Riesgo residual (ya existía, no lo introduce este cambio):** el fallback solo actúa si el histórico se actualizó dentro de los 3 días (`STALE_VOID_DAYS`) previos a la expiración, y el backfill es semanal (`BACKFILL_ALL.bat`). Un backfill diario aumentaría la cobertura. Es una decisión operativa que no se tomó aquí.
