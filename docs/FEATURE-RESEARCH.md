...
candidato. Es un borrador, no un experimento preregistrado: aún faltan inicio,
fin y tamaño de muestra/potencia. No reutilizar el período de descubrimiento
como confirmación independiente.
...
```powershell
python -B -m pytest -q -p no:cacheprovider --basetemp=.codex-tmp/pytest-features tests/test_feature_research.py tests/test_measure_features_harness.py tests/test_feature_store.py tests/test_feature_manifest.py tests/test_mlb_features.py tests/test_ml_models.py tests/test_compare.py
