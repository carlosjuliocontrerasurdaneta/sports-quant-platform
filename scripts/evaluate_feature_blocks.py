    ...
    for league in leagues:
        try:
            ...
            if meta["family"] == "baseball" and fip_path.exists():
                fip_bytes = fip_path.read_bytes()
                fips = pd.read_csv(BytesIO(fip_bytes), dtype={"game_id": str})
                ...
                fip_hash = hashlib.sha256(fip_bytes).hexdigest()
            snapshots = None
            if args.snapshots_dir:
                sp = args.snapshots_dir / f"{league}.csv"
                if sp.exists():
                    snapshot_bytes = sp.read_bytes()
                    snapshots = pd.read_csv(BytesIO(snapshot_bytes), dtype={"game_id": str})
