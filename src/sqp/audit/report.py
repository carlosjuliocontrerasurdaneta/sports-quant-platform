...
def consolidated_report(predictions_dir: Path | None = None, top: int = 100) -> str:
    ...
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    day = ts[:8]
    # Que entra: los picks VIGENTES -- el partido no se ha jugado --, no los del
    # ultimo dia de generacion. Las ligas no se refrescan todas cada dia (el
    # guardian de presupuesto aplazo 14 el 2026-08-27) y un run que cruza la
    # medianoche parte los candidatos en dos dias, asi que filtrar por generacion
    # borraba de la lista picks perfectamente validos: el 2026-08-28 el reporte
    # salio con una sola liga. Lo que si hay que decir es de cuando es cada fila.
    df = picks_vigentes(load_all_candidates(predictions_dir, generated_day=None))
    hoy_local = local_today()
    aviso = ""
    if not df.empty and "generated_at" in df.columns:
        dias = df["generated_at"].astype(str).str[:10]
        df = df.copy()
        df["generado"] = dias
        viejos = sorted(set(dias[dias != hoy_local]))
        if viejos:
            aviso = (f"> **ATENCION: hay candidatos del {', '.join(viejos)}, "
                     f"no de hoy ({hoy_local}).** Su partido sigue por jugar, pero "
                     f"la cuota es la de ese momento; ver la columna `generado`.")
